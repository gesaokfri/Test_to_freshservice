...
namespace App\Controllers\Integration\RumahSakit;
...
class IntegrationFinanceRumahSakitController extends BaseController
{
  ...
  public function Index()
  {
    ...
    $group = "03"; // 01 = Yayasan, 02 = Unika, 03 = Rumah Sakit
    
    if ($group == 01) { $groupName = "Yayasan "; } 
    elseif ($group == 02) { $groupName = "UNIKA "; } 
    elseif ($group == 03) { $groupName = "Rumah Sakit "; } 
    else { $groupName = ""; }
  ...
  public function getPendapatanQuarter($monthYEAR_req, $group_req)
  {
    ...
    $getList  = $db->query("
    select 
      SUM ( glbalance.PERIOD_NET_DR - glbalance.PERIOD_NET_CR ) AS Total, GLCODEDESC.FLEX_VALUE as COA, glcodecombinations.SEGMENT1 AS GroupData, GLCODEDESC.DESCRIPTION AS Descr,
    ...
    GROUP BY
      GLCODEDESC.FLEX_VALUE, glcodecombinations.SEGMENT1, GLCODEDESC.DESCRIPTION, glbalance.PERIOD_NAME");
    ...
    foreach ($getList->getResult() as $list) {
      ...
      if (!in_array($list->COA, $excludeCOA)) {
        ...
        $total += $list->Total;
        // echo "&emsp;&emsp;" . $list->Descr . " -> " . $list->Total. " | Total Pendapatan (Penambahan) setelah foreach : " . int_to_rp($total) . "<br>";
      }
    }
  ...
  public function getAsetLancar($monthYEAR_req, $group_req)
  {
    ...
    FROM
		  ...
		  INNER JOIN [StagingData].[dbo].[AJ_GL_BALANCES_V] AS glbalance ON glcodecombinations.CODE_COMBINATION_ID = glbalance.CODE_COMBINATION_ID 
	  WHERE
      glcodecombinations.SEGMENT5 BETWEEN '1101001' and '1201001' and glcodecombinations.SEGMENT5 NOT IN ('1201001', '1106998') and glcodecombinations.SEGMENT1='" . $group . "' and glbalance.PERIOD_NAME= '" . $period . "' and glbalance.ACTUAL_FLAG= 'A' 
    GROUP BY
      glcodecombinations.SEGMENT5, glcodecombinations.SEGMENT1, GLCODEDESC.DESCRIPTION, glbalance.PERIOD_NAME");
  ...
  public function getAsetTidakLancar($monthYEAR_req, $group_req)
  {
    ...
    FROM
		  ...
		  INNER JOIN [StagingData].[dbo].[AJ_GL_BALANCES_V] AS glbalance ON glcodecombinations.CODE_COMBINATION_ID = glbalance.CODE_COMBINATION_ID 
	  WHERE 
      glcodecombinations.SEGMENT5 BETWEEN '1201002' and '1401004' and glcodecombinations.SEGMENT1='" . $group . "' and glbalance.PERIOD_NAME= '" . $period . "' and glbalance.ACTUAL_FLAG= 'A'
    GROUP BY
  ...
  public function getLiabilitasJangkaPendek($monthYEAR_req, $group_req)
  {
    ...
    FROM
      ...
      or glcodecombinations.SEGMENT5 = '9000001' ) 
  ...
  public function getCapex($monthYEAR_req, $group_req)
  {
    ...
    $group  = $group_req;      //01 = Yayasan, 02 = Unika
    $period = $monthYEAR_req;  // JAN-18
    if ($group_req == 01) { $groupName = "Yayasan "; } elseif ($group_req == 02) { $groupName = "UNIKA ";
    } elseif ($group_req == 03) { $groupName = "RUMAH SAKIT "; } else { $groupName = ""; }
