...
class AnggaranController extends BaseController
{
  ...
  public function Index(){
    if (!$this->session->has('session_id')) {
      ...
      if(acc_read(session('level_id'),$data['id_menu'])=="1") {
        return $this->blade->render("yayasan/pages/anggaran/index", $data);
      } else {
  ...
  public function ViewUpload(){
    if (!$this->session->has('session_id')) {
      ...
      if(acc_read(session('level_id'),$data['id_menu'])=="1") {
        return $this->blade->render("yayasan/pages/anggaran/upload", $data);
      } else {
        return redirect()->to('/yayasan');
      }
    }
  ...
  public function Res(){
    if (!$this->session->has('session_id')) {
      ...
      $data['id_menu'] = $this->mydata['id_menu'];
      if (acc_read(session("level_id"), $data["id_menu"]) == "1") {
        return $this->blade->render("yayasan/pages/anggaran/res", $data);
      } else {
        return redirect()->to('/yayasan');
      }
    }
  ...
  public function Read(){
    ...
    $query   = $tableAnggaran->select('ms_budget.*')->where('budget_group','2');
  ...
  public function Edit() {
    if (!$this->session->has('session_id')) {
      ...
      return $this->blade->render("yayasan/pages/anggaran/edit", $data);
    }
  }
  ...
  public function Update(){
    ...
    $check = $tableAnggaran->where('budget_group',2)->where('budget_id', $id)->first();
    if(!empty($check)){
        $data = [
  ...
  public function Import()
  {
    if (!$this->session->has('session_id')) {
      ...
      return $this->blade->render("yayasan/pages/anggaran/import");
    }
  }
  ...
  public function ImportInsertData_Preview()
  {
    ...
    if (isset($_FILES['excel_file']['name'])) {
      ...
      return $this->blade->render("yayasan/pages/anggaran/import_insert_anggaran_preview", $data);
    }
  }
