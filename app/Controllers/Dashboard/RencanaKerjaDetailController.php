  ...
  public function Res(){
    if (!$this->session->has('session_id')) {
      ...
      if (acc_read(session("level_id"), $data["id_menu"]) == "1") {
        ...
        return $this->blade->render("pages.rencana_kerja_detail.container", $data);
      } else {
    ...
    foreach ($getList as $list) {
      ...
      if(acc_delete(session('level_id'),$id_menu)=="1"){
        ...
        if($list->rencana_kerja_verifikasi=="0") { // Pending
          $list_btn .= "<li>
                        <button class='btn btn-delete btn-sm text-nowrap m-1' onclick='action_delete(\"".$list->rencana_kerja_detail_id."\", \"Hapus Detail Rencana Kerja \", \"Apakah anda yakin ingin hapus data <br/> Detail Rencana Kerja ?\", \"warning\", \"" . base_url() . "/universitas/rencana_kerja_detail/delete\",refresh)'><i class='mdi mdi-delete-outline font-size-16'></i> Hapus</button>
                      </li>"; 
         }
      }
